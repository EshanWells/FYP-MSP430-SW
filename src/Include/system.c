...
void initDCOClock(enum DCO_FREQ dcoFreq)
{
    ...
    switch (dcoFreq) {
        ...
        BCSCTL |= XT2OFF; //ensure XT2 off.

        __delay_cycles(2000); //Safe time to wait for the DCO to stabilise. Can be shortened in a pinch
    }
}
...
{
    BCSCTL3 = 0x00;
    BCSCTL3 |= XCAP3; //set caps to 12.5pF
}
